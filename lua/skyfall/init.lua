...
local config = require("skyfall.config")
local utils = require("skyfall.utils")
local bufferline = require("skyfall.integrations.bufferline")
...
local function set_groups()
    ...
    CursorInsert = { link = "Cursor" },
    CursorReplace = { link = "Cursor" },
		lCursor = { link = "Cursor" },
		...
		PmenuSel = { bg = colors.selection, fg = colors.selectionFg },
		...
		Visual = { bg = utils.shade(colors.skyfall, 0.10) },
		...
		Keyword = { fg = colors.keyword, italic = config.italics.keywords or false },
		...
		["@variable"] = { fg = colors.variable, italic = config.italics.variables or false },
		["@variable.builtin"] = { fg = colors.variable, italic = config.italics.variables or false },
...
function theme.colorscheme()
	if vim.version().minor < 8 then
		vim.notify("Neovim 0.8+ is required for skyfall colorscheme", vim.log.levels.ERROR, { title = "Min Theme" })
		return
	end
